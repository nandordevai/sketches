const sketch = new p5((p5) => {
    const isDesktop = p5.windowWidth > 480;
    const ratio = isDesktop ? 1 : .5;
    const width = Math.min(
        1200 * ratio,
        2000 //document.querySelector('#home')?.getBoundingClientRect().width - (isDesktop ? 405 : 0)
    );
    ...
    class Shape {
        constructor() {
            ...
            this.start = p5.createVector(
                p5.random(width),
                p5.random(-50 * ratio, -100 * ratio)
            );
            this.lengths = [
                p5.random(width * .6, width * .9) * ratio,
